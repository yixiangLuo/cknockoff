...
cKnockoff_j <- function(y, X, X.pack,
    ...
    for(mc_i in 1:mc_size){
        ...
        # Ej_mc[mc_i] <- (g_star - w_tilde * alpha) * sample_weights[mc_i]
        # if(mc_i %% 2 ==0){
        #     ineq_combine <- (Ej_mc[seq(1, mc_i-1, length.out = mc_i/2)] + Ej_mc[seq(2, mc_i, length.out = mc_i/2)]) / 2
        #
...
ineq_invest <- invest_ineq(n, p, mc_size, X_type = "Sparse", target, alpha,
...
# results <- lapply(1:sample_size, function(iter){
  ...
  full_select <- cknockoff(X, y,
                           ...
                           n_cores = 1,
                           X.pack = X.pack,
                           Rhat_refine = F)$selected
  
  y.pack <- process_y(X.pack, y, randomize = F)
  kn_stats_obs <- statistic(X, X.pack$X_kn, y,
                            sigma_tilde = y.pack$sigmahat_XXk_res)
  
  Rhat_select <- cknockoff_Rhat(X.pack, y, j_exclude = NULL,
                           kn_stats_obs,
                           y.pack$sigmahat_XXk_res,
                           statistic,
                           alpha,
                           Rhat_max_try = 5,
                           Rhat_calc_max_step = 4)$selected
  
  return(list(full = full_select, Rhat = Rhat_select))
}

inconsistent_sel_num <- lapply(results, function(res){
  length(setdiff(res$Rhat, res$full))
})

inconsistent_sel_num <- sum(unlist(inconsistent_sel_num))
total_Rhat_sel <- lapply(results, function(res){
  length(res$Rhat)
})
total_Rhat_sel <- sum(unlist(total_Rhat_sel))

print(paste(inconsistent_sel_num, "out of", total_Rhat_sel, "selection in Rhat inconsistent with cKnockoff"))
```
...
result <- cknockoff(X, y, alpha = 0.05, n_cores = 1, Rhat_refine = T)
print(result$selected)
