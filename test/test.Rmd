...
cKnockoff_j <- function(y, X, X.pack,
                        ...
                        invest_mode = T, invest_j = NULL,
                        sample_type = "random", naive_sampler = T,
                        kn_reg_method = "local_lin_reg",
                        statistic, cali_var = "pval"){
    # X <- X_invest
    # y <- y_invest
    ...
    init_selected <- j %in% union(kn_selected, Bonf_selected)
    ...
    kn_stat_nodes <- kn_stat_sampling(kappa * alpha, j, y.pack,
    ...
    if(cali_var != "pval"){
      ...
      cali_stat_samples <- cali_stat_sampling(j, y.pack,
                                              X.pack = list(X = X.pack$X, X_kn = X.pack$X_kn,
                                                   vj = X.pack$vj_mat[, j],
                                                   X_res_Xk_basis = X.pack$X_res_Xk_basis,
                                                   XXk_res_unit = X.pack$XXk_res_unit),
      ...
      cali_rej_set <- where_cali_rej(j, y.pack,
                                     X.pack = NA,
                                     statistic,
                                     method = kn_reg_method, cali_var,
                                     cali_thr = cali_thr,
                                     cali_stat_samples = cali_stat_samples,
                                     y_fit_noj_all = NA)
      
      kn_stat_nodes <- list(vjy_nodes = cali_stat_samples$vjy_nodes,
                            kn_abs_stat_j = cali_stat_samples$cali_stat,
                            kn_stat_thrs = rep(cali_thr, 10))
      kn_rej_set <- cali_rej_set
    }
    
    pval_rej_set <- where_pval_rej(j, y.pack)
    sample_region <- interval_union(kn_rej_set, pval_rej_set)
    ...
    for(mc_i in 1:mc_size){
        ...
        kn_result <- kn.select(kn_stats, kappa * alpha,
                                selective = T, early_stop = 1)
        kn_selected_RB <- kn_result$selected

        # compute weight
        kn_null_num <- (kn_result$fdp_est * length(kn_selected_RB)) %>% max(1)
        w_tilde <- (j %in% kn_selected_RB) / kn_null_num * kappa

        # for investing
        if(invest_mode){
            eskn_rej_mc[mc_i, ] <- (1:p) %in% kn_selected_RB
            kn_stat_thres[mc_i] <- kn_result$W_k_hat
            pi0_mc[mc_i] <- kn_result$fdp_est
            Reskn_mc[mc_i] <- length(kn_selected_RB)
            
            if(cali_var != "pval"){
              kn_stat_thres[mc_i] <- NA
            }
        }
        ...
        # p-values
        if(naive_sampler){
            tvals_RB <- lm_to_t(y_cond[, mc_i], X)$tvals
        } else{
        ...
        # for investing
        if(invest_mode){
            Rkn_mc[mc_i] <- length(kn_selected_RB)
        }

        # R hat
        selected_RB <- union(kn_selected_RB, Bonf_selected_RB)

        # compute marginal selection
        Mg_selected_RB <- ifelse(pvals_RB[j] <= pval_obs, j, 0)
        # if(kappa < 1){
        #   Mg_selected_RB <- ifelse(pvals_RB[j] <= pval_obs, j, 0)
        # } else{
        #   Mg_selected_RB <- ifelse((pvals_RB[j] <= pval_obs) &&
        #                            kn_prefer(j, kn_stats, alpha), j, 0)
        # }
        if(cali_var != "pval"){
          cali_stats_mc <- cali_statasitic(X, X.pack$X_kn, y_cond[, mc_i],
                                          statistic, sigma_tilde = sigmahat_XXk_res[mc_i],
                                          y_fit_noj_all, type = cali_var)
          Mg_selected_RB <- ifelse(cali_stats_mc[j] >= cali_stats_obs[j], j, 0)
        }

        # compute g star
        g_star <- (j %in% union(kn_selected_RB, Mg_selected_RB)) / length(union(selected_RB, j))

        ineq_LR[, mc_i] <- c(g_star, w_tilde * alpha) * sample_weights[mc_i] + c(0, 1/p * (1-kappa) * alpha)
        # for investing
        if(invest_mode){

            pval_invest <- pvals_t(tvals_RB, df, side = "right")
            ...
            p_ineq <- add_row(p_ineq,
                              ...
                              ineq_R = (w_tilde + 1/p * (1-kappa)) * alpha,
                              kn_rej = (j %in% kn_selected_RB),
                              nokn_rej = (j %in% Mg_selected_RB),
                              kept = x_in_intervals(tvals_RB[j], sample_region))
            ...
            if(cali_var != "pval"){
              kn_stats_mc[mc_i, ] <- cali_stats_mc
            }
        }

        # if(mc_i %% 2 == 0){
        #
        #     ineq_in_rej <- (p_ineq$ineq_L[1:mc_i] - p_ineq$ineq_R[1:mc_i]) * sample_weights[1:mc_i] - (1-kappa) * alpha / p
        ...
        # ineq[mc_i] <- (g_star - w_tilde * alpha) * sample_weights[mc_i] - (1-kappa) * alpha / p
        # if(mc_i %% 2 ==0){
    ...
    ineq_in_rej <- (p_ineq$ineq_L[inds_in_rej] - p_ineq$ineq_R[inds_in_rej]) * sample_weights[inds_in_rej] * num_in_rej / mc_size - (1-kappa) * alpha / p
    ...
    return(list(p_ineq = p_ineq, pval_mat = pval_mat, kn_stats_mc = kn_stats_mc,
                ...
                SRL_est_mc = SRL_est_mc, kn_stat_thres = kn_stat_thres,
                qval_invest = qval_invest, sample_weights = sample_weights,
                pi0_mc = pi0_mc, Rkn_mc = Rkn_mc, Reskn_mc = Reskn_mc,
                init_selected = init_selected, ineq_selected = ineq_selected,
                kn_rej_set = kn_rej_set, nokn_rej_reg = pval_rej_set,
...
invest_ineq <- function(n, p, mc_size, X_type, target, alpha, alt_num, kappa,
                        y_seed, invest_j, import_ratio = NULL,
                        sample_type = "random", naive_sampler = T,
                        kn_reg_method = "local_lin_reg",
    ...
    ineq_invest <- cKnockoff_j(y, X, X.pack,
                               ...
                               H0, seed = y_seed+100,
                               mc_size = mc_size,
                               invest_mode = T, invest_j = invest_j,
                               sample_type = sample_type, naive_sampler = naive_sampler,
                               kn_reg_method = kn_reg_method,
                               statistic = statistic, cali_var = cali_var)
...
draw_kn_stat <- function(ineq_invest, invest_j, SRL = F){
    ...
    data.frame(ineq_invest$kn_stats_mc) %>%
        ...
        geom_line(data = data.frame(vjy = ineq_invest$vjy_mc,
                                    kn_stat_thres = abs(ineq_invest$kn_stat_thres),
                                    hypo = invest_j,
...
nonzero <- sample(p, k)
beta <- 0.5 * (1:p %in% nonzero)
y <- X %*% beta + rnorm(n)
print(which(1:p %in% nonzero))

# Basic usage
result <- cknockoff(X, y, alpha = 0.05, n_cores = 1, cali_var = "pval")
print(result$selected)
result <- cknockoff(X, y, alpha = 0.05, n_cores = 1, cali_var = "LM_lasso")
print(result$selected)
